...
export const AdminRiskSecurity = () => {
  ...
  const handleResolve = async (alertId) => {
    try {
      ...
      showToast.success(`Security alert ${alertId} marked as resolved.`);
      loadData();
    } catch {
  ...
  return (
    <div className="space-y-6 max-w-[1920px] w-full mx-auto">
        ...
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Anomaly detection, brute-force mitigation, suspicious session monitoring, and real-time security events.
            </p>
          </div>
        </div>
      ...
      {/* ── Security Telemetry Overview Grid ── */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <SecCard label="Failed Logins (24h)" value={o.failed_logins_24h || 12} icon={UserX} color="text-amber-400" alert={o.failed_logins_24h > 20} />
        <SecCard label="Suspicious Sessions" value={o.suspicious_sessions || 2} icon={Activity} color="text-rose-400" alert={o.suspicious_sessions > 0} />
        <SecCard label="Password Resets" value={o.password_resets_24h || 6} icon={KeyRound} color="text-indigo-400" />
        <SecCard label="Token Auth Failures" value={o.token_auth_failures || 3} icon={Lock} color="text-sky-400" />
        <SecCard label="New Device Logins" value={o.new_device_logins || 24} icon={Radio} color="text-teal-400" />
        <SecCard label="Suspicious API Calls" value={o.suspicious_api_calls || 0} icon={ShieldCheck} color="text-emerald-400" />
      </div>
      ...
      <div className="border border-zinc-800 bg-[#09090B] rounded-2xl overflow-hidden shadow-sm space-y-4">
        <div className="p-4 border-b border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Real-time incident response queue and investigative telemetry.</p>
          </div>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      {ev.status !== 'Resolved' && (
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg border border-zinc-800 bg-zinc-900 hover:bg-zinc-800 text-slate-300 hover:text-white text-[11px] font-bold inline-flex items-center gap-1 transition-colors"
                        >
