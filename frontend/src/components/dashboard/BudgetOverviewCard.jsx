...
const SkeletonCard = () => (
  <div className="animate-pulse space-y-4">
    <div className="grid grid-cols-3 gap-3">
      {[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-zinc-800 rounded-2xl" />)}
    </div>
...
export const BudgetOverviewCard = ({ budgetOverview, loading }) => {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-base font-bold text-white">Budget Overview</h3>
        <p className="text-xs text-slate-400">Active budgets this period</p>
      </div>

      {/* Summary stats */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Active', value: total_active_budgets, color: 'text-indigo-400' },
          { label: 'Exceeded', value: exceeded_budgets, color: 'text-rose-400' },
          { label: 'On Track', value: on_track_budgets, color: 'text-emerald-400' },
        ].map((s) => (
      ...
      <div className="rounded-2xl bg-[#121216] p-4 shadow-md">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm font-black text-white">{Number(overall_utilization_pct).toFixed(1)}%</span>
        </div>
        <ProgressBar
      ...
      {/* Budget list */}
      {budgets.length === 0 ? (
        <p className="text-sm text-slate-400 text-center py-4">No active budgets.</p>
      ) : (
        <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
          {budgets.map((b) => (
            <div key={b.id} className="rounded-2xl bg-[#121216] p-3 shadow-md">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="text-xs font-bold text-white truncate max-w-[140px]">{b.budget_name}</p>
                  {b.category_name && (
                    <p className="text-[10px] text-slate-400">{b.category_name}</p>
                  )}
                </div>
