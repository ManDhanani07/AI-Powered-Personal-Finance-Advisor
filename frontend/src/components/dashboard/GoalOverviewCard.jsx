...
const PriorityBadge = ({ priority }) => {
  ...
  }[priority] || 'bg-zinc-800 text-slate-400';
...
export const GoalOverviewCard = ({ goalsOverview, loading }) => {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-base font-bold text-white">Goals Overview</h3>
        <p className="text-xs text-slate-400">Your savings journey progress</p>
      </div>

      {/* Summary stats */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total', value: total_goals, color: 'text-slate-200' },
          { label: 'Active', value: active_goals, color: 'text-indigo-400' },
          { label: 'Achieved', value: completed_goals, color: 'text-emerald-400' },
        ].map((s) => (
          <div key={s.label} className="rounded-2xl bg-[#121216] p-3 text-center shadow-md">
      ...
      {nearest_deadline_goal && (
        <div className="rounded-2xl bg-[#121216] p-4 shadow-md">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-white truncate">{nearest_deadline_goal.goal_name}</p>
              <p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5">
                <Calendar className="h-3 w-3" />
                {formatDate(nearest_deadline_goal.target_date)} · {nearest_deadline_goal.days_remaining}d left
              </p>
              <p className="text-[11px] text-slate-400 mt-1">
                Need: <span className="font-semibold text-amber-400">{formatCompactFinancial(nearest_deadline_goal.required_monthly_saving)}/mo</span>
              </p>
            </div>
      ...
      {activeGoals.length === 0 ? (
        <p className="text-sm text-slate-400 text-center py-3">No active goals.</p>
      ) : (
        <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
          {activeGoals.map((g) => {
            ...
            return (
              <div key={g.id} className="rounded-2xl bg-[#121216] p-3 flex items-center gap-3 shadow-md">
                <RadialProgress pct={pct} color={color} size={52} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs font-bold text-white truncate">{g.goal_name}</p>
